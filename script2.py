...
def compare_collections(
    ...
    try:
        ...
        if content_differences:
            logger.info(f"Content differences for common _ids: {json.dumps(content_differences[:2], indent=2, cls=JSONEncoder)}")  # Log first 2 for brevity
...
def main():
    ...
    # Track collections with no differences
    no_differences = []
    ...
    for collection in collections:
        ...
        has_differences = (
            differences['missing_in_source'] or
            differences['missing_in_target'] or
            differences['content_differences']
        )

        if has_differences:
            # Save differences to a file in the output directory
            output_file = os.path.join(output_dir, f"differences_{collection_name}_{timestamp}.json")
            ...
            logger.info(f"Differences saved to {output_file}")
        else:
            # Add to no_differences list
            no_differences.append({
                'collection_name': collection_name,
                'common_count': differences['common_count']
            })

    # Save no_differences to a single file in the output directory
    if no_differences:
        no_differences_file = os.path.join(output_dir, f"no_differences_{timestamp}.json")
        with open(no_differences_file, 'w') as f:
            json.dump(
                no_differences,
                f,
                indent=2,
        ...
        logger.info(f"No differences for {len(no_differences)} collections saved to {no_differences_file}")
